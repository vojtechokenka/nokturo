...
export function PageShell({ children, headerSlot, actionsSlot, bare = false, compactContent = false, noHorizontalPadding = false, noContentPadding = false, contentBg = 'default', contentRounded = true, contentOverflow = 'auto' }: PageShellProps) {
  if (bare) {
    return (
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden scrollbar-hide" data-scroll-container>
          {children ?? null}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
      ...
      <div className={`flex-1 min-h-0 overflow-hidden flex flex-col ${contentRounded ? 'rounded-none sm:rounded-[12px]' : 'rounded-none'} ${contentBg === 'black' ? 'bg-surface' : 'bg-white/5'}`}>
        <div
